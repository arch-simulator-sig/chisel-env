...
class Rename extends Module with nanshanConfig {
...
class RenameTable extends Module with nanshanConfig {
  val io = IO(new Bundle {
    val en = Input(Bool())
    // rs1, rs2
...
class PrfStateTable extends Module with PrfStateConstant with nanshanConfig {
  val io = IO(new Bundle {
    val en = Input(Bool())
